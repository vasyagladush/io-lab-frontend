...
export const AddNewAccessPoint: React.FunctionComponent<{
  ...
  const handleNetworkSelect = (
    ...
    if (isAnythingModified && !avoidIsModified) {
      showDialog({
        ...
        content:
          "You have unsaved changes for this access point and network. Please save them.",
        actions: [
          {
            type: "outline",
            text: "Cancel",
  ...
  const onSubmit = async (values: AccessPointFormValues) => {
    try {
      ...
      if (editing && accessPointId) {
        await updateAccessPoint(accessPointId, values);
        handleRefresh();
      }
    } catch (e: any) {
      console.error(e);
  ...
  const handleRefresh = async () => {
    try {
      ...
      await refreshNetworkInfo(String(currentDefaultNetworkValues?.id));
          ...
          {loadingNetwork ? (
            ...
            <>
              ...
              <NetworkSecurityConfig
                control={control}
                macACL={currentNetworkFields?.security?.macACL}
              />
            </>
          )}
        </RightSide>
